...
@Component({
  selector: 'app-attempted-report',
  templateUrl: './attempted-report.component.html',
  styleUrls: ['./attempted-report.component.css']
})
export class AttemptedReportComponent implements OnInit {
  quizResultList=[];
  ...
  ngOnInit(): void {
    ...
    if(sessionStorage.getItem("ssid")){
      ...
      this.quizResultList = JSON.parse(JSON.stringify(user.quizResultList))
  ...
  getResult(quiz){
    sessionStorage.setItem("resultId", JSON.stringify(quiz))
